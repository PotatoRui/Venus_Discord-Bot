...
exports.run = async(client, message, args, config, guild) => {
    ...
    function embedbal(date, daily, voicechannelspend, uid) {
        ...
        let embed = new Discord.MessageEmbed()
            ...
            .setColor("#ffdf00")
            .setFooter('The Shack PH ™ | Daily Voice Quest', "https://cdn.discordapp.com/attachments/739149373308534784/746488384612073593/1596097943017_copy_387x387.png")
        
        var ohr,thr,rhr;
        if(Number(voicechannelspend) >= 3600000 && Number(voicechannelspend) <= 7199999) {
            ohr = "★ ~~1 Hour of Call\nReward: 50 Copper~~\n";
            thr = "☆ **2 Hours of Call**\nReward: **1 Silver and 50 Copper**\n";
            rhr = "☆ **3 Hours of Call**\nReward: **3 Silvers**\n";
            checkuserquestlogs(uid.id,"v1hr","50c");
        } else if(Number(voicechannelspend) >= 7200000 && Number(voicechannelspend) <= 10799999) {
            ohr = "★ ~~1 Hour of Call\nReward: 50 Copper~~\n";
            thr = "★ ~~2 Hours of Call\nReward: 1 Silver and 50 Copper~~\n";
            rhr = "☆ **3 Hours of Call**\nReward: **3 Silvers**\n";
            checkuserquestlogs(uid.id,"v1hr","50c");
            checkuserquestlogs(uid.id,"v2hr","150c");
        } else if(Number(voicechannelspend) > 10800000) {
            ohr = "★ ~~1 Hour of Call\nReward: 50 Copper~~\n";
            thr = "★ ~~2 Hours of Call\nReward: 1 Silver and 50 Copper~~\n";
            rhr = "★ ~~3 Hours of Call\nReward: 3 Silver~~\n";
            checkuserquestlogs(uid.id,"v1hr","50c");
        ...
        embed.setDescription(`────────  ⋅⋆ ─ ⋆⋅  ────────\n`+
                            ...
                            `${rhr}`+
                            `─────────────────────\n`+
                            `• Total Voice Time: ${humanizeDuration(voicechannelspend)}\n`+
                            `────────  ⋅⋆ ─ ⋆⋅  ────────`)
    ...
    function checkuserquestlogs(uid,type,amount) {
        ...
        var sql = 'SELECT * FROM quest_voice WHERE userid = ? AND type = ?';
        db.query(sql, [uid,type], function(err, result) {
            if (err) throw err;

            if (result.length === 0) {
                var sql = 'INSERT INTO quest_voice (userid,type) VALUES (?,?)';
